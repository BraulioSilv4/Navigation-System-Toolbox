...
# Exercise 2
# a) z -> 8x1 vector of pseudorange measurements (one for each satellite)
# b) H -> 8x4
# c) x -> 4x1  our position (x, y, z, clock bias)
# d) H^T -> 4x8
# e) H^T * H -> 4x4
# f) (H^T * H)^-1 -> 4x4
# g) (H^T * H)^-1 * H^T -> 4x1
...
for i in range(3600):
    for svn, eph in orb:
        ...
        if elevation > 10.0:
            ...
            dict_svn_range_with_offset[svn] = dict_svn_range_with_offset.get(svn, []) + [pseudorange]
...
pseudo_ranges: list[float] = [npr.pseudoranges[0] for npr in npr_data]
orb_above_10_deg = []
for svn, eph in orb:
...
position, estimate, _ = LS(guess=r3, prange_sats=prange_sats, trx=536400.0, max_iter=10, debug=True)
print(f"Estimated position: {position}, clock bias: {estimate:.4f} m")
...
for size in range(4, len(prange_sats) + 1):
    ...
    for combo in combinations(prange_sats, size):
        _, _, H = LS(guess=r3, prange_sats=list(combo), trx=536400.0, max_iter=10, debug=False)
