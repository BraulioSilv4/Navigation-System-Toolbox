...
class LLH:
    ...
    def orthodrome(self, target: LLH, radius: float):
        lat1 = radians(self.coordinates.lat.to_dd())
        lon1 = radians(self.coordinates.lon.to_dd())
        lat2 = radians(target.coordinates.lat.to_dd())
        lon2 = radians(target.coordinates.lon.to_dd())

        rad_distance = math.acos(cos(lat2) * cos(lon1 - lon2) * cos(lat1) + sin(lat2) * sin(lat1))
        rad_depart_heading = math.atan2((-cos(lat2) * sin(lon1 - lon2)), (-cos(lat2) * cos(lon1 - lon2) * sin(lat1) + sin(lat2) * cos(lat1)))
        rad_approach_heading = math.atan2((-sin(lon1 - lon2) * cos(lat1)), (sin(lat2) * cos(lon1 - lon2) * cos(lat1) - cos(lat2) * sin(lat1)))

        return (rad_distance * radius), math.degrees(rad_depart_heading), math.degrees(rad_approach_heading)

    def loxodrome(self, target: LLH, radius: float):
        lat1 = radians(self.coordinates.lat.to_dd())
        lon1 = radians(self.coordinates.lon.to_dd())
        lat2 = radians(target.coordinates.lat.to_dd())
        lon2 = radians(target.coordinates.lon.to_dd())

        sigma1 = math.log(math.tan(math.pi / 4 + lat1 / 2))
        sigma2 = math.log(math.tan(math.pi / 4 + lat2 / 2))

        delta_phi = lat2 - lat1
        delta_lam = lon2 - lon1
        delta_sigma = sigma2 - sigma1

        if abs(delta_lam) > math.pi:
            if delta_lam > 0:
                delta_lam = -(2 * math.pi - delta_lam)
            else:
                delta_lam = (2 * math.pi + delta_lam)

        psi = math.atan2(delta_lam, delta_sigma)

        if abs(delta_phi) < 1e-12:
            distance = radius * math.cos(lat1) * abs(delta_lam)
        else:
