...
def LS_ex3(guess: ECEF, prange_sats: list[tuple[float, Ephemeris]], trx, max_iter: int = 10, debug: bool = False):
    r = guess.to_vector()
    ...
    for i in range(max_iter):
        ...
        for prange, sat in prange_sats:
            sat_pos_raw, _ = sat.sat_position_at_tx(r=ECEF(r[0], r[1], r[2]), t_rx=(604800 - sat.toe) + trx, debug=debug)
        ...
        niter += 1
        if np.linalg.norm(r_new - r) < 0.001:   # Error calculated based on the previous estimate and the current
            return ECEF(r_new[0], r_new[1], r_new[2]), estimate[3][0], H, niter

        r = r_new

    return ECEF(r[0], r[1], r[2]), estimate[3][0], H, niter
